...
import { Activity, Brain, Swords, Zap, CalendarDays, CheckCircle2 } from 'lucide-react'
import Link from 'next/link'
import QuestCard from '@/components/QuestCard'

// TypeScript Interface to eliminate "is possibly null" errors
type QuestData = {
...
export default async function CommandCenter() {
  ...
  const rawQuests = (allFetchedNodes || []).map(node => {
    ...
    if (isGym || isProtocol) {
      return {
        ...
        nodeId: node.id,
        questIndex: 0,
        nodeTitle: node.skill_trees?.title || (isGym ? 'Gym Training' : 'Daily Routine'),
        title: node.title,
        nodeDescription: node.description || 'Complete the prescribed routine.',
        attribute: attr,
        nodeReward: node.xp_reward || 0,
        dailyReward: calculatedReward,
      ...
      return {
        id: `${node.id}-${activeIndex}`,
        nodeId: node.id,
        questIndex: activeIndex,
        nodeTitle: node.title,
        title: subQuests[activeIndex].title,
  ...
  // Strongly typed filter to solve the TS null errors
  const allQuests = rawQuests.filter((q): q is QuestData => q !== null)

  const hasTitle = (q: QuestData, titles: string[]) => titles.some(t => (q.nodeTitle + ' ' + q.title).toLowerCase().includes(t.toLowerCase()))

  const intAcadQuests = allQuests.filter(q => q.attribute === 'INT' && hasTitle(q, ['foundation', 'object-oriented', 'data structure', 'operating system']))
  const intOtherQuests = allQuests.filter(q => q.attribute === 'INT' && !hasTitle(q, ['foundation', 'object-oriented', 'data structure', 'operating system']))
  
  const groomingQuests = allQuests.filter(q => q.attribute === 'STR' && hasTitle(q, ['protocol', 'maintenance', 'nutrition', 'food', 'macro']))
  // ONLY fetch the Gym quest whose title includes today's actual day name (e.g., "Monday")
  const gymQuests = allQuests.filter(q => q.attribute === 'STR' && !!q.exercises && q.title.toLowerCase().includes(currentDayName.toLowerCase()))
  
  const duoQuests = allQuests.filter(q => hasTitle(q, ['spanish']))
  const guitarQuests = allQuests.filter(q => q.attribute === 'DEX' && hasTitle(q, ['fretboard', 'campfire', 'electric']))
  const readingQuests = allQuests.filter(q => hasTitle(q, ['literary', 'escapism', 'reading']))
  const artQuests = allQuests.filter(q => q.attribute === 'DEX' && hasTitle(q, ['ui/ux', '3d modeling', 'video production']))
  const chaQuests = allQuests.filter(q => q.attribute === 'CHA')

  const scheduledQuests: QuestData[] = []

  scheduledQuests.push(...intAcadQuests.slice(0, 2))
  scheduledQuests.push(...duoQuests.slice(0, 1))
  scheduledQuests.push(...groomingQuests)
  
  if (dayOfWeek !== 0) scheduledQuests.push(...gymQuests.slice(0, 1))
  
  scheduledQuests.push(...intOtherQuests.slice(0, 1))
  
  if (dayOfWeek % 2 === 0 && dayOfWeek !== 0) scheduledQuests.push(...chaQuests.slice(0, 1))
  if (dayOfWeek % 2 !== 0) scheduledQuests.push(...guitarQuests.slice(0, 1))
  if (dayOfMonth === 1 || dayOfMonth === 15) scheduledQuests.push(...readingQuests.slice(0, 1))
  if (dayOfMonth === 1) scheduledQuests.push(...artQuests.slice(0, 1))

  const finalQuests = scheduledQuests
    .slice(0, 10)
    .sort((a, b) => Number(a.isActuallyDone) - Number(b.isActuallyDone))

  const activeCount = finalQuests.filter(q => !q.isActuallyDone).length
